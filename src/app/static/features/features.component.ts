...
export class FeaturesComponent implements OnInit {
  ...
  click(e: string) {
    this.objectVariablesObject[e].chosen = !this.objectVariablesObject[e]
      .chosen;
  ...
  searchQuestionsByWords() {
    let finalString = this.objectVariables
    ...
    if (this.checked) {
      this.httpService
        .findByVariablesAnd(finalString)
        .subscribe((data: any[]) => {
          //this.ref.markForCheck();
          console.log(data);
          this.questions = data.map(d => {
            return new VariablesShort(
              d.Name,
              d.Label,
              d.Type,
              d.Study,
